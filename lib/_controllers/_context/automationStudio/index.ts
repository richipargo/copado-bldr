    ...
    switch (req) {
      case "search":
        ...
        if (typeof argv.f === "string" && argv.f.includes(":")) {
          ...
          allowTracking() &&
            incrementMetric(
              `req_searches_automationStudio_${contentType}_folders`
            );
          ...
          allowTracking() &&
            incrementMetric(
              `req_searches_automationStudio_automations_folders`
            );
        }
        /**
        ...
        if (typeof argv.a === "string" && argv.a.includes(":")) {
          ...
          const searchTerm = argv._ && argv._[1];
          const searchRequest = await automationStudio.searchActivity(
            activity,
            searchTerm
          );
      ...
      case "clone":
        ...
        if (typeof argv.f === "string" && argv.f.includes(":")) {
          ...
          const searchRequest =
            await automationStudio.gatherAutomationDefinitionsByCategoryId({
              contentType,
              categoryId,
            });
          ...
          const formattedAssetResponse =
            ...
            assets.map((asset: any) => {
              const category = folders.find(
                (folder: { ID: number }) => folder.ID === asset.categoryId
              );
          ...
          allowTracking() &&
            incrementMetric(
              `req_clones_automationStudio_${contentType}_folders`
            );
        ...
        if (typeof argv.a === "string" && argv.a.includes(":")) {
          ...
          const searchRequest =
            await automationStudio.gatherAutomationDefinitionsById({
              contentType,
              assetId,
            });
...
const processAutomationCloneRequest = async (cloneAutomationRequest: {
  ...
  cloneAutomationRequest &&
    ...
    (await createAutomationStudioEditableFiles(
      cloneAutomationRequest.formattedAutomationDefinitions
    ));
  ...
  displayLine(
    `>> Cloned ${cloneAutomationRequest.formattedAutomationDefinitions.length} Definitions`
  );

  // Create Automation Dependencies
  Object.keys(cloneAutomationRequest.formattedAutomationDependencies) &&
    Object.keys(cloneAutomationRequest.formattedAutomationDependencies).forEach(
      async (context: any) => {
        displayLine(`Cloning Dependencies: ${context}`, "info");

        const contextDependencies =
          cloneAutomationRequest.formattedAutomationDependencies[context];
        contextDependencies &&
        ...
        displayLine(
          `>> Cloned ${contextDependencies.assets.length} ${context} Dependencies`
        );
      }
    );
};
