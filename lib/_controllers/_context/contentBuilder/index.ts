    ...
    switch (req) {
      case "search":
          ...
          if (typeof argv.f === "string" && argv.f.includes(":")) {
            ...
            switch (searchFlag) {
              case "shared":
                ...
                allowTracking() &&
                  incrementMetric("req_searches_sharedContent_folders");

                break;
            }
          } else if (
            (typeof argv.f === "string" && !argv.f.includes(":")) ||
            argv.f === "number"
          ) {
            searchRequest =
              (await contentBuilder.searchFolders({
            ...
            allowTracking() &&
              incrementMetric("req_searches_contentBuilder_folders");
          }
        }
        ...
        if (argv.a) {
          ...
          allowTracking() &&
            incrementMetric("req_searches_contentBuilder_assets");
        }
        break;
      ...
      case "clone":
        ...
        if (argv.f) {
          if (typeof argv.f === "string" && argv.f.includes(":")) {
            ...
            if (!cloneRequest.assets.length) {
              displayLine(
                "No assets returned, folder is likely empty or does not exist.",
                "info"
              );
              return;
            }
            ...
            allowTracking() &&
              incrementMetric("req_clones_sharedContent_folders");
          } else if (
            (typeof argv.f === "string" && !argv.f.includes(":")) ||
            typeof argv.f === "number"
          ) {
            ...
            allowTracking() &&
              incrementMetric("req_clones_contentBuilder_folders");
          }
        }
        ...
        if (argv.a) {
          ...
          if (typeof argv.a === "string" && argv.a.includes(":")) {
            ...
            const isolatedFoldersUnique =
              folders && uniqueArrayByKey(folders, "id");

            assets &&
              assets.length &&
              (await createContentBuilderEditableFiles(assets));
            assets &&
              folders &&
              (await updateManifest("sharedContent", {
                assets: assets,
            ...
            allowTracking() &&
              incrementMetric("req_clones_sharedContent_assets");
          } else if (
            (typeof argv.a === "string" && !argv.a.includes(":")) ||
            ...
            if (
              ...
              displayLine(
                `Unable to Clone Request, asset might not exist`,
                "error"
              );
              return;
            }
            ...
            allowTracking() &&
              incrementMetric("req_clones_contentBuilder_assets");
          }
        }
        break;
      ...
      case "delete":
        if (argv.f) {
          ...
          const assetIds =
            assets && assets.length && assets.map((asset) => asset.id);
          let folderIds =
            (folders &&
              folders.length &&
              folders
        ...
        if (argv.a) {
          if (argv["force"]) {
            ...
            if (deleteRequest === "OK") {
              ...
              allowTracking() &&
                incrementMetric("req_deletes_contentBuilder_assets");
            }
          } else {
            yargsInteractive()
              ...
              .then(async (initResults) => {
                if (initResults.confirmDelete) {
                  const deleteRequest = await bldr.sfmc.asset.deleteAsset(
                    argv.a
                  );
                  if (deleteRequest === "OK") {
                    displayLine(
                      `AssetId ${argv.a} has been deleted`,
                      "success"
                    );
                    displayLine(
                      `Please Note: folders have not been deleted. Working on it though!`,
