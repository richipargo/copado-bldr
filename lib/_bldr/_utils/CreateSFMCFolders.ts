...
const addNewFolders = async (
  ...
  try {
    ...
    const manifestAssetCategories = manifestJSON[folder.context.context][
      "assets"
    ].map(
      (manifestAsset: { category: { folderPath: string } }) =>
        manifestAsset && manifestAsset.category
    );
    let manifestFolderCategories = manifestJSON[folder.context.context][
      "folders"
    ].map((manifestFolder: { folderPath: string }) => manifestFolder);
    ...
    for (const stashItemFolder in stashItemFolderArray) {
      ...
      manifestFolderCategories = manifestJSON[folder.context.context][
        "folders"
      ].map((manifestFolder: { folderPath: string }) => manifestFolder);
      ...
      if (folderIndex === -1) {
        if (typeof parentId === "undefined") {
          ...
          await updateManifest(folder.context.context, {
            folders: [parentFolderObject],
          });
        ...
        if (
          !createFolder ||
          (typeof createFolder === "string" &&
            createFolder.includes("Please select a different Name."))
        ) {
          ...
          do {
            const newFolderId =
              (createFolder &&
                ...
                createFolder.Results[0].NewID) ||
              null;

            if (newFolderId) {
              displayLine(
                `${folderName} has been created; CategoryId: ${newFolderId}`,
                "success"
              );
...
const addExistingFolderToManifest = async (
  ...
  if (
    ...
    const folderObject = results.find(
      (folderResult: { Name: string }) => folderResult.Name === request.folder
    );
