...
const sfmc_context = (systemFilePath: string) =>
  sfmc_context_mapping.filter((context: { name: string }) =>
    new RegExp(context.name).test(systemFilePath)
  );
/**
 *
 * @param systemFilePath
    ...
    projectDir:
      (contextDetails &&
        ...
        (isWindows()
          ...
          : parsedFilePath.dir.substring(
              parsedFilePath.dir.indexOf(contextDetails[0].name)
            ))) ||
      null,
