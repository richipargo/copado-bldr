...
export class Initiate {
  ...
  updateKeys = async () => {
    try {
      ...
      fs.writeFileSync(
        path.join(`${rootPath}.copado.manifest.json`),
  ...
  initiateContentBuilderProject = async () => {
    ...
    const dirEmpty =
      dirExists && (await isDirEmpty(`${rootPath}content-builder`));
    ...
    if (!dirExists || dirEmpty) {
      ...
      allowTracking() &&
        incrementMetric("req_project_initiates_contentBuilder");
    } else {
      ...
      .then(async (initResults) => {
        ...
        if (initResults.sendableDataExtension) {
          ...
          dataExtensionInit.sendableDataExtensionField = {
            ...
            fieldType:
              "{{ field type of field to use in sendable relationship }}",
          };
        ...
        await createFile(
          `${initFolderPath}/${initResults.dataExtensionName}.json`,
          dataExtensionInit
        );
        allowTracking() &&
          incrementMetric("req_project_initiates_dataExtension");
      });
  };
}
