...
export class Push {
  ...
  pushStash = async () => {
    ...
    const availableContexts = await uniqueArrayByKey(
      availableContextsArray,
      "context"
    );
    ...
    for (const context in availableContexts) {
      ...
      const contextStash = instanceStash.filter(
        (stashItem) => stashItem.bldr.context.context === currentContext
      );
      ...
      debug(
        `${postStashFiles.length}  Files to be Created`,
        "info",
        postStashFiles
      );
      ...
      debug(
        `${putStashFiles.length} Files to be Updated`,
        "info",
        putStashFiles
      );
  ...
  pushToSFMC = async (
    ...
    try {
      ...
      if (!sdk) {
        displayLine(
          "Unable to initiate BLDR SDK. Please review credentials and retry.",
          "error"
        );
        return;
      }
      ...
      for (let f = 0; f < stashFiles.length; f++) {
        ...
        const folderPath =
          stashFileObject.bldr && stashFileObject.bldr.folderPath;
        const stashFileContext =
          stashFileObject.bldr && stashFileObject.bldr.context.context;
        const method = Object.prototype.hasOwnProperty.call(
          stashFileObject.bldr,
          "id"
        )
          ? "put"
          : "post";
        ...
        if (sfmcUpdateObject) {
          ...
          const stashFileObjectReplaced = await replaceBldrSfmcEnv(
            JSON.stringify(stashFileObject)
          );
          ...
          switch (stashFileContext) {
            case "automationStudio":
              if (method === "put") {
                ...
                assetResponse = await sdk.sfmc.automation.patchAutomationAsset(
                  sfmcAPIObject
                );
              ...
              if (
                ...
                assetResponse.response.data.errors &&
                  ...
                  assetResponse.response.data.errors.forEach(
            ...
            case "sharedContent":
              ...
              manifestContextFolders =
                manifestJSON[
                  stashFileContext === "contentBuilder"
                    ? "contentBuilder"
                    : "sharedContent"
                ] &&
                manifestJSON[
            ...
            case "dataExtension":
              ...
              if (method === "put") {
                ...
                assetResponse = await sdk.sfmc.emailStudio.postAsset(
                  sfmcAPIObject
                );
              ...
              if (
                ...
                Object.prototype.hasOwnProperty.call(
                  assetResponse.Results[0],
                  "Object"
                ) &&
                Object.prototype.hasOwnProperty.call(
                  assetResponse.Results[0]["Object"],
                  "CustomerKey"
                )
              ) {
                ...
                await createFile(
                  `${folderPath}/${sfmcAPIObject.name}.json`,
                  sfmcAPIObject
                );
                success.push(sfmcAPIObject);
              } else {
      ...
      err.response.data &&
        ...
        displayLine(err.response.data.message, "error");
      err.response.data &&
      ...
      err.response.data &&
        ...
        err.response.data.validationErrors.length &&
        displayLine(
          `ErrorCode: ${err.response.data.validationErrors[0].errorcode}`,
          "error"
        );
    }
  };
