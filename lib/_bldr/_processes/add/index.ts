...
export class Add {
  ...
  addAllFiles = async (packageManifestJSON = null) => {
    try {
      ...
      if (!packageManifestJSON) {
        const organizedFiles = await this.gatherAllFiles(
        ...
        const organizedFiles = await this.gatherAllFilesFromPackage(
          contextFiles
        );
  ...
  gatherAllFiles = async (contextFiles: string[], rootPath2: string) => {
    ...
    const manifestPath =
      rootPath && path.join(rootPath, ".copado.manifest.json");
    ...
    for (const context in availableContexts) {
      const folderNameRegex = new RegExp(
        "[\\\\/]+" + availableContexts[context].name + "[\\\\/]+",
        "i"
      );
      const contextPaths = contextFiles.filter((file) =>
        folderNameRegex.test(file)
      );
      ...
      if (manifestContextAssets) {
        ...
        for (const path in contextPaths) {
          ...
          const existingAsset = manifestContextAssets.find((asset) => {
            ...
            return (
              formattedDir.endsWith(asset.category.folderPath) &&
              name === asset.name &&
              asset
            );
          });
          ...
          if (existingAsset) {
            ...
            debug(
              "existing - fileContentRaw",
              "info",
              fileContentRaw || "nothing here"
            );
            ...
            const existingSchema: {
              ...
              bldr: {
                ...
                folderPath:
                  existingAsset.category && existingAsset.category.folderPath,
              },
              fileContent,
            };
            ...
            const { name, dirName, dir, formattedDir, projectDir } =
              await getFilePathDetails(systemFilePath);
            const fileContentRaw = await readFile(systemFilePath);
            let fileContent = fileContentRaw.toString();
            debug(
              "new - fileContentRaw",
  ...
  gatherAllFilesFromPackage = async (contextFiles: string[]) => {
    ...
    for (const context in availableContexts) {
      const contextPaths = contextFiles.filter((file) =>
        file.includes(availableContexts[context].name)
      );
      const bldrContext = availableContexts[context].context;
        ...
        for (const path in contextPaths) {
          ...
          const existingSchema: StashItem = {
            ...
            bldr: {
              ...
              folderPath:
                packageAsset.category && packageAsset.category.folderPath,
            },
            fileContent,
      ...
      .then(async (optionsResult) => {
        try {
          ...
          for (const resultBldrId in optionsResult) {
            ...
            if (postFile) {
              ...
              if (optionsResult[resultBldrId]) {
                postFile.assetType = MappingByAssetType(
          ...
          const noOptionFiles = request.postFiles.filter(
            (noOptionPost: any) =>
              !Object.keys(optionsResult).includes(noOptionPost.bldr.bldrId)
          );
