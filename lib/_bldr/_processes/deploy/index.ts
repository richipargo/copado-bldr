...
export class Deploy {
  ...
  deployPackage = async (argv: Argv) => {
    try {
      ...
      const availableContexts: string[] = sfmcContext.sfmc_context_mapping.map(
        (ctx) => ctx.context
      );

      const packageContexts = Object.keys(packageJSON).map((key) => {
        return (
          availableContexts.includes(key) && typeof key === "string" && key
        );
      });
      ...
      for (const c in packageContexts) {
        ...
        if (context && packageJSON[context]) {
          ...
          for (const fp in pkgFolderPaths) {
            const ctxDetails = sfmcContext.sfmc_context_mapping.find(
      ...
      sfmcOnly && fs.unlinkSync("./.copado.manifest.json");
    } catch (err) {
      console.log(err);
    }
  };
  ...
  deployCheckConfig = async () => {
    ...
    if (fileExists(`${dirPath}/copado/copado.json`)) {
   ...
   */
  deployContentBuilderAssets = async (
    sdk: BLDR_Client,
    ...
    try {
      ...
      for (const d in dataExtensions) {
        ...
        const manifestJSONFolder = manifestJSON["dataExtension"][
          ...
          (folder: ManifestFolder) =>
            folder.folderPath === dataExtension.category.folderPath && folder
        );
        ...
        const createDataExtension = await sdk.sfmc.emailStudio.postAsset(
          dataExtension
        );
      ...
      statusMessage &&
        statusMessage.includes(
          "Updating an existing Data Extension definition"
        ) &&
        displayLine(
          "Please ensure all Data Extension names/customer keys are unique",
          "error"
        );
    }
  };
