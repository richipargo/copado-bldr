...
export class Package {
  ...
  packageConfig = async () => {
    try {
      ...
      if (!fileExists(`${dirPath}.copado.manifest.json`)) {
        throw new Error(
        ...
        .then(
          async (initResults: {
            ...
            try {
              ...
              const tagsSplit =
                (initResults.tags && initResults.tags.split(",")) || [];
              ...
              for (const c in availableContexts) {
                ...
                contextAssets = await replaceBldrSfmcEnv(
                  JSON.stringify(contextAssets)
                );
                ...
                switch (context) {
                  case "contentBuilder":
                    ...
                    const newContextKeys =
                      (newDependencies && Object.keys(newDependencies)) || [];

                    newContextKeys &&
                      newContextKeys.length &&
                      displayLine(
                        `Creating files for ${newContextKeys.join(", ")}`,
                        "info"
                      );

                    for (const k in newContextKeys) {
                      displayLine(
                        `Working on ${newContextKeys[k]}`,
                        "progress"
                      );
                      let newAssets =
                        newDependencies[newContextKeys[k]]["assets"];
                      await createEditableFilesBasedOnContext(
                        newContextKeys[k],
                        newAssets
                      );
                    }
              ...
              packageOut?.contentBuilder?.assets.forEach(
                (asset: { id?: number; exists?: Boolean }) => {
                  delete asset.id;
                  delete asset.exists;
                }
              );
              ...
              let packageOutRendered = await scrubBldrSfmcEnv(
                JSON.stringify(packageOut)
              );
              packageOutRendered = JSON.parse(packageOutRendered);

              await createFile(
                "./package.manifest.json",
                JSON.stringify(packageOutRendered, null, 2)
              );
