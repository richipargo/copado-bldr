...
const createAutomationStudioEditableFiles = async (assets: any[]) => {
  try {
    for (const a in assets) {
      ...
      let folderPath =
        asset.category.folderPath || (assetType && assetType.folder);

      if (asset && asset.hasBeenDeleted && asset.hasBeenDeleted === true) {
        displayLine(
          `Error Creating File [local | ${assetTypeName}]: Automation Definition Deleted`,
          "error"
        );
        return;
      }
      ...
      createFileResult &&
        displayLine(
          `Successfully Created [local | ${assetTypeName}]: ${
      ...
      !createFileResult &&
        displayLine(
          `Error Creating File [local | ${assetTypeName}]: ${
            asset.name || asset.Name
          }`,
          "error"
        );
