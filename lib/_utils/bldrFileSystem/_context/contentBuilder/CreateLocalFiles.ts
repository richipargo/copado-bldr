 ...
 * @param assets
 */
const createContentBuilderEditableFiles = async (
  ...
  try {
    for (const a in assets) {
      ...
      const folderPath =
        (asset && asset.category && asset.category.folderPath) || null;
      const id = asset.id;
      ...
      createFileResult &&
        displayLine(`Successfully Created [local]: ${asset.name}`, "success");
      !createFileResult &&
        displayLine(`Error Creating File [local]: ${asset.name}`, "error");
    }
  } catch (err: any) {
