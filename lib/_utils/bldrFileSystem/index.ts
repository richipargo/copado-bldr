...
const normalizedENVPath = path.join(normalizedRoot, "/copado/copado.json");
const normalizedTemplateENVPath = path.join(
  normalizedRoot,
  "/copado/copado.json"
);
const normalizedGitIgnorePath = path.join(normalizedRoot, ".gitignore");
const normalizedManifestJSONPath = path.join(
  normalizedRoot,
  ".copado.manifest.json"
);
const normalizedPackageManifestJSONPath = path.join(
  normalizedRoot,
  ...
  if (fileExists(envPath)) {
    ...
    for (const c in config) {
      ...
      if (content.match(key)) {
        content =
          (value && value !== "" && content.replace(`{{${key}}}`, value)) ||
          content;
      }
    }
  }
...
const createAllDirectories = (folderPaths: { folderPath: string }[]) => {
  const directories = folderPaths.map(({ folderPath }) =>
