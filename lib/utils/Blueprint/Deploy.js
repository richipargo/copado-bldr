...
module.exports = class Deploy {
    ...
    try {
      ...
      for (const c in contexts) {
        ...
        if (packageJSON[context]) {
          ...
          const pkgFolders = pkgAssets.map(
            (asset) => asset.category.folderPath
          );

          //Deploy all Folders
          const createFolders = await this.deployFolders(
            pkgFolders,
            contextDetails
          );
  ...
  async deployContentBuilderAsset(
    ...
    const categoryDetails = manifestJSONFolders.find(
      (folders) => folders.folderPath === contentFolderPath
    );
    ...
    contentBuilderAsset = await utils.updateAssetContent(
      contentBuilderAsset,
      buildContent
    );
      ...
      for (const d in dataExtensions) {
        ...
        const manifestJSONFolder = manifestJSON["dataExtension"][
          ...
          (manifestJSONFolderFolderObj) =>
            manifestJSONFolderFolderObj.folderPath ===
            dataExtension.category.folderPath
        );
        ...
        if (
          ...
          throw new Error(
            `${createDataExtension.Object.Name}: ${createDataExtension.StatusMessage}`
          );
        }
