...
module.exports = class Config {
      ...
      yargsInteractive()
        ...
        .then(async (configResults) => {
          ...
          const configured = {
            ...
            apiClientSecret: await crypto.encrypt(
  ...
  async get(instance, show) {
    ...
    if (config && show) {
      ...
      const displayContent = [
        [new Column(`${JSON.stringify(config, null, 2)}`, width.c3)],
      ];
  ...
  async remove(argv) {
    try {
      ...
      const displayContent = [
        [new Column(`${JSON.stringify(configurationResp, null, 2)}`, width.c3)],
      ];
    ...
    try {
      ...
      if (ignoreError) {
        const displayContent = [
          [
            new Column(
              `${styles.callout("bldr caught a configuration error:")}`,
              width.c3
            ),
          ],
  ...
  _displayInstanceNames(config) {
    ...
    if (configArray.length !== 0) {
      ...
      const displayContent = [
        [new Column(`${styles.callout("No Configured Instances")}`, width.c2)],
      ];
