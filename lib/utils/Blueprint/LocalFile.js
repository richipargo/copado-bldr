...
module.exports = class LocalFile {
  ...
  _getSFMCConfig() {
    ...
    if (this._fileExists(`${dirPath}/copado/copado.json`)) {
      const config = fs.readFileSync(`${dirPath}/copado/copado.json`);
  ...
  _getManifest(dirPath) {
    const manifest = fs.readFileSync(`${dirPath}.copado.manifest.json`);
  ...
  async createEditableFiles(assets, context, bldr = true) {
    try {
      for (const a in assets) {
        ...
        const assetType =
          (asset.assetType && asset.assetType.name) || asset.ContentType;
        ...
        const id =
          asset.id ||
          (objectIdKey && asset[objectIdKey]) ||
          (asset.customerKey && asset.customerKey);
  ...
  appendBLDR(obj, rootPath) {
    ...
    if (this._fileExists(bldrPath)) {
      ...
      const exists =
        bldrJSON.findIndex(({ id }) => id === obj.id) !== -1 ? true : false;
  ...
  async manifestJSON(context, content, dirPath) {
    ...
    if (this._fileExists(manifestPath)) {
        ...
        if (
          ...
          for (const i in items) {
            ...
            if (Object.prototype.hasOwnProperty.call(item, "id")) {
              ...
              const objectIdKey =
                (item && item.assetType && item.assetType.objectIdKey) || null;
      ...
      console.log("log in localFile.js");
