...
module.exports = class Status {
  ...
  async status() {
    ...
    const stateHeaders = [
      new Column(`${styles.command("Current State")}`, width.c4),
    ];

    display.render(stateHeaders, [
      [new Column(`${JSON.stringify(state, null, 2)}`, width.c4)],
    ]);
    ...
    if (stashArr && stashArr.length) {
      ...
      const headers = [
        new Column(`${styles.command("Staged Files")}`, width.c4),
      ];

      display.render(headers, [
        [new Column(`${styles.callout("No Files Staged")}`, width.c4)],
      ]);
    }
  }
};
