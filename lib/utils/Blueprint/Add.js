        ...
        try {
          ...
          for (const o in optionsResult) {
            ...
            if (typeof postFile !== "undefined") {
              ...
              manifestFolder = await this.stash._getManifestFolderData(
                postFile
              );
              ...
              if (ignoreFolderCreate.includes(assetType)) {
                ...
                const categoryResp = await this.bldr.folder.search(
                  assetType,
                  "Name",
                  parentName
                );
              ...
              switch (assetType) {
                ...
                case "htmlblock":
                  postObj = await assetDefinitions.contentBlock(
                    asset,
                    assetType
                  );
                  break;
          ...
          const errorHeaders = [
            new Column(`${styles.error("New Asset Error")}`, width.c3),
          ];
