...
module.exports.migrate = async () => {
  ...
  for (const s in savedConfigurations) {
    ...
    configuration.apiClientId = await decryptExisting(
      configuration.apiClientId
    );
    configuration.apiClientSecret = await decryptExisting(
      configuration.apiClientSecret
    );

    configuration.apiClientId = await updatedCrypto.encrypt(
      configuration.apiClientId
    );
    configuration.apiClientSecret = await updatedCrypto.encrypt(
      configuration.apiClientSecret
    );
    ...
    if (setPsw) {
      ...
      console.log(
        `${account}: There was an error with migration. Please reconfigure using [bldr config -n].`
      );
    }
  }
};
