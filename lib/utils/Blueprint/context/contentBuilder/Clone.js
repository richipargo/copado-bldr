...
module.exports = class CBClone {
  ...
  async init() {
    ...
    const isDirEmpty = await !this.localFile._fileExists(
    ...
    if (isDirEmpty) {
      ...
      const displayContent = [
        [
          new Column(
            `${styles.callout("Root directory must be empty")}`,
            width.c3
          ),
        ],
      ];
  ...
  async cloneFromFolder(argv, context) {
    try {
      ...
      if (checkRootFolder.OverallStatus === "OK") {
        ...
        const parentFolderIndex = pathToRootArray.indexOf(
          rootFolderObj.ParentFolder.Name
        );
      ...
      if (folderPaths.length === 0)
        throw new Error(`No Folder Paths Found for ${styles.detail(id)}`);
  ...
  async cloneFromID(id, obj = null, legacy = false) {
    try {
      ...
      await this.localFile.manifestJSON(
        "contentBuilder",
        { folders: getFolderPaths.formattedFolders },
        null
      );
  ...
  async setFolderPathRecursiveAsc(folderObj) {
    ...
    const formattedFolders = await this._formatFolderResponse(
      foundFolders,
      folderObj.id
    );
  ...
  _formatFolderResponse(folders, id) {
    if (folders && folders.length !== 0) {
      const map = folders.map((folder) => {
        if (
          folder.Name !== "smfcRepo/content-builder" ||
          folder.name !== "smfcRepo/content-builder"
        ) {
