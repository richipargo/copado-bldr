...
module.exports = class ASSearch {
  ...
  async dataFolder(contentType, searchKey, searchTerm) {
    try {
      ...
      if (!resp.Results || !resp.Results.length)
        throw new Error(
          `No Search Items Returned for ${styles.detail(searchTerm)}`
        );
  ...
  async asset(searchKey, searchTerm) {
    try {
      ...
      if (
        Object.prototype.hasOwnProperty.call(resp, "Results") &&
  ...
  async activity(searchActivity, searchTerm) {
    try {
      const resp = await this.bldr.automation.searchActivity(
        searchActivity,
        searchTerm
      );
