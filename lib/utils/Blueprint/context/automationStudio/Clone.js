...
module.exports = class ASClone {
  ...
  async cloneAutomationsFromFolder(argv, context) {
    try {
      ...
      const assetsResp = await this.bldr.automation.getByFolderArray(
        folderIds,
        this.cb_clone
      );
    ...
    try {
      ...
      for (const a in assets) {
        ...
        if (folderObj) {
          ...
          folderObj = await this._formatFolderResponse(
            folderResp.Results,
            asset.categoryId
          );
  ...
  async cloneAutomationActivityFromID(assetType, id) {
    try {
      let assetResp = await this.bldr.automation.getAutomationActivity(
        assetType,
        id
      );
      assetResp.assetType =
        await utils.identifyAutomationStudioActivityObjectTypeId(assetType);
      const folderPaths = new Array();
  ...
  async _formatDataForFile(results, folderPaths) {
    if (Array.isArray(results) && results.length !== 0) {
      ...
      for (const r in results) {
        ...
        const folderPath =
          (searchPath && searchPath.folderPath) ||
