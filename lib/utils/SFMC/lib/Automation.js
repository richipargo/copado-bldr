...
module.exports = class Automation {
  ...
  async getByFolderArray(folderIds, cb_clone) {
    try {
      ...
      const automationStepDefinitions = await this.getAutomationActivities(
        automations,
        cb_clone
      );
  ...
  async getAutomationsByKey(automationKeys) {
    ...
    if (Array.isArray(automationKeys)) {
      ...
      const autoRequest = await this.rest.get(
        `/automation/v1/automations/${automationKeys}`
      );
        ...
        if (Object.prototype.hasOwnProperty.call(automation, "steps")) {
          for (const as in automation.steps) {
            ...
            for (const sa in steps.activities) {
              ...
              const assetType =
                await utils.identifyAutomationStudioActivityType(
              ...
              if (assetType) {
                ...
                if (assetType.name === "userinitiatedsend") {
                  ...
                  if (
                    soapResp.OverallStatus !== "OK" ||
                    soapResp.Results.length === 0
                  ) {
                  ...
                  const emailBldrId = await cb_clone.cloneFromID(
                    legacyId,
                    null,
                    true
                  );
                  ...
                  if (assetType.name === "filteractivity") {
                    ...
                    if (filterDefinition)
                      stepActivity.filterDefinition = filterDefinition;
                  }
                }
  ...
  async getAutomationActivity(assetType, activityObjectId) {
    try {
      const resp = await this.rest.get(
        `/automation/v1/${assetType}/${activityObjectId}`
      );
