...
module.exports.init = async (mid, store) => {
  try {
    ...
    if (
      !configurationObject.apiClientId.includes("@|@") ||
      !configurationObject.apiClientSecret.includes("@|@")
    ) {
      throw new Error(
        `Configuration for ${instance} needs to be updated. Please run 'bldr config --encrypt'`
      );
    }
    ...
    return new SFMC(
      {
        ...
        client_secret: await crypto.decrypt(
          configurationObject.apiClientSecret
        ),
    ...
    const displayHeader = [
      new Column(`${styles.error("SFMC API Authentication Error")}`, width.c3),
    ];
