...
module.exports.getAllFiles = async () => {
  ...
  const contexts = contextMap
    .map((ctx) => localFiles._fileExists(`./${ctx.root}`) && ctx.root)
    .filter(Boolean);
 ...
 */
module.exports.formatContentBuilderDataForFile = async (
  bldr,
  results,
  folderPaths
) => {
 ...
 * @returns
 */
module.exports.identifyAutomationStudioActivityObjectTypeId = (
  activityType
) => {
  let out;
...
module.exports.getAssetContent = (asset) => {
  const assetType =
    (asset && asset.assetType && asset.assetType.name) || "null";
  let content;
